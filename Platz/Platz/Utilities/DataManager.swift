...
class DataManager: ObservableObject {
    ...
    // MARK: - Lesson Helpers
    func lessons(forTrack track: LessonTrack) -> [Lesson] {
        lessons.filter { $0.track == track }.sorted { $0.order < $1.order }
    }
    ...
    func lessonsCount(forTrack track: LessonTrack) -> Int {
        lessons(forTrack: track).count
    }
    
    // MARK: - Dialog Helpers
    func dialogs(forTopic topic: String) -> [Dialog] {
        dialogs.filter { $0.topic == topic }.sorted { $0.order < $1.order }
    }
    ...
    var allDialogs: [Dialog] {
        dialogs.sorted { $0.order < $1.order }
    }

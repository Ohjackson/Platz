...
#### 데이터 소스
- `lessons.json` - 4개 트랙에 걸친 15개 구조화된 레슨
- `dialogs.json` - 5개 회화 시나리오 (인사, 카페, 길찾기, 쇼핑, 호텔)
- `quizzes.json` - 설명이 포함된 15개 이상의 퀴즈
...
│   ├── lessons.json       # 15개 언어 학습 레슨
│   ├── dialogs.json       # 5개 회화 시나리오
│   ├── quizzes.json       # 15개 이상의 퀴즈 세트
...
**구성**: 15개 레슨
- 알파벳 트랙 3개 (특수문자, A-J, K-Z)
- 숫자 트랙 3개 (1-20, 21-99, 100 이상)
- 문법 트랙 3개 (sein 동사, 관사, haben 동사)
- 회화 트랙 3개 (인사, 안부, 감사/사과)
...
**구성**: 5개 대화
1. 첫 만남 인사 (`dialog_greeting_01`)
2. 카페에서 주문하기 (`dialog_cafe_01`)
3. 길 묻기 (`dialog_directions_01`)
4. 빵집에서 (`dialog_bakery_01`)
5. 호텔 체크인 (`dialog_hotel_01`)
